...
with st.expander("📊 آخر التعديلات"):
    if not st.session_state.products_db.empty:
        ...
        db_copy['display_text'] = db_copy.apply(lambda r: f"{r['اسم النوع']} ({r['الكود']}) ... آخر تعديل: {r['تاريخ آخر تعديل']}", axis=1)
        st.selectbox("استعراض القائمة الحالية:", options=db_copy['display_text'], label_visibility="collapsed")
    else:
...
# -------------------------------------------------------------
# 🔍 خانة البحث والنتائج التلقائية للمرور والعمال
# -------------------------------------------------------------
...
if search_query:
    ...
    filtered_df = df[df['اسم النوع'].str.contains(search_query, case=False, na=False) | df['الكود'].astype(str).str.contains(search_query, case=False, na=False)]
    ...
    if not filtered_df.empty:
        for index, row in filtered_df.iterrows():
            with st.container(border=True):
                ...
                with col2: st.text_input(label="اللون المتاح", value=row['اللون'], disabled=True, key=f"src_col_{index}")
                with col3: st.text_input(label="مكان التخزين", value=row['المخزن'], disabled=True, key=f"src_loc_{index}")
...
# -------------------------------------------------------------
# 🛠️ واجهة المسؤول (تظهر فقط عند كتابة كود 404)
# -------------------------------------------------------------
if password_input == ADMIN_PASSWORD:
        ...
        if st.button("💾 حفظ التعديلات وتحديث التواريخ"):
            ...
            if len(edited_df) == len(st.session_state.products_db):
                for i in range(len(edited_df)):
                    ...
                    if not old_r.equals(new_r): edited_df.at[i, "تاريخ آخر تعديل"] = current_time
            else:
    ...
    with admin_tabs[2]:
        ...
        uploaded_file = st.file_uploader("اختر ملف الإكسيل:", type=["xlsx", "xls"])
            ...
            try:
                ...
                if not missing_cols:
                    if st.button("🚀 اعتماد وتنظيم الإكسيل فوراً"):
                        ...
                        st.success("🎉 تم جلب البيانات وتنظيمها بالكامل!")
                        st.rerun()
                else:
    ...
    # 4. الترتيب والتسمية
    with admin_tabs[3]:
        st.subheader("⚙️ ترتيب ظهور البيانات")
        if not st.session_state.products_db.empty:
            sort_by = st.selectbox("ترتيب حسب:", ["اسم النوع", "الكود", "العدد المتوفر"])
            ascending_choice = st.radio("الاتجاه:", ["تصاعدي", "تنازلي"])
            
            if st.button("🔄 تطبيق الترتيب الجديد"):

...
if 'db_main' not in st.session_state:
    st.session_state.db_main = pd.DataFrame([
        {"الكود": "101", "اسم النوع": "دم غزال", "عدد الامتار": 150.0, "العدد": 3, "المكان": "العشة", "التاريخ": "15/6/2026", "الوقت": "3:00"}
    ])
...
# زر الترس العلوي وقفل الجلسة (تم إصلاح الكود هنا تماماً)
...
with t_c1:
    if st.button("⚙️"):
        if st.session_state.is_admin:
            ...
            st.rerun()
            # شريط الدخول السري للمسؤول كود 404 المدمج في المستطيل
...
# الهيدر المحدث بـ "المخازن" فقط بدون الصندوق المزعج لجميع المستخدمين
...
# شريط التنقل العلوي بين النظامين
app_page = st.radio("", ["🖥️ عرض بيانات المخزن", "📤 تسجيل الخارج من المخزن"], horizontal=True, label_visibility="collapsed")
st.write("---")

# [الصفحة الأولى]: عرض بيانات المخزن الحالي للعمال والمرور بالأعمدة الجديدة بالترتيب
if app_page == "🖥️ عرض بيانات المخزن" and not st.session_state.is_admin:
    ...
    with st.expander("آخر الإضافات"):
        for idx, row in df.iterrows():
            ...
            # [الصفحة الثانية]: تسجيل الخارج من المخزن مع إضافة "اسم التسليم" بنفس الترتيب والأعمدة المتساوية
    ...
    with st.form("out_form", clear_on_submit=True):
        ...
        if st.form_submit_button("🚀 تأكيد تسجيل الخروج وحفظ العملية", use_container_width=True):
            ...
            else: st.markdown("<p style='text-align:center; color:red;'>⚠️ يرجى تعبئة الكود والاسم واسم التسليم.</p>", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

# [لوحة المسؤول المصلحة]: إدارة المخازن بالتحديث الفوري والربط الصحيح
        ...
        with st.form("add_m"):
            ...
            if st.form_submit_button("➕ حفظ للجدول", use_container_width=True):
                if nc and nn:
                    ...
                    st.success("✅ تم إضافة الصنف الجديد!")
                    st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
            ...
            try:
                ...
                if not [c for c in req if c not in up_df.columns]:
                    if st.button("🚀 تفريغ واعتماد الإكسيل فوراً", use_container_width=True):
                        ...
                        st.success("🎉 تم تفريغ الإكسيل وتحديث مخازن زقزوق!")
                        st.rerun()
